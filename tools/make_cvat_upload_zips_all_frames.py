...
CVAT_DIR = DATASET / "03_cvat_upload_all"
INV_DIR = DATASET / "00_inventory"

CVAT_DIR.mkdir(parents=True, exist_ok=True)
INV_DIR.mkdir(parents=True, exist_ok=True)

MANIFEST_CSV = INV_DIR / "cvat_upload_all_manifest.csv"
SUMMARY_TXT = INV_DIR / "cvat_upload_all_summary.txt"

# 2000 ảnh / zip để CVAT đỡ nặng
IMAGES_PER_ZIP = 2000
...
def main():
    print("==================================================")
    print("STEP 5/20 - MAKE CVAT ZIP PACKAGES")
    print("==================================================")
    ...
    if not FRAMES_ROOT.exists():
        print(f"[ERROR] Khong tim thay folder frame: {FRAMES_ROOT}")
        raise SystemExit(1)
    ...
    if total_images == 0:
        print("[ERROR] Khong tim thay anh .jpg/.png nao.")
        raise SystemExit(1)

    print(f"[OK] Total images found: {total_images}")

    # Xoa zip cu neu co
    old_zips = list(CVAT_DIR.glob("cvat_phone_radio_all_*.zip"))
    for z in old_zips:
        z.unlink()
    ...
    for start in range(0, total_images, IMAGES_PER_ZIP):
        ...
        zip_name = f"cvat_phone_radio_all_{package_index:03d}.zip"
        zip_path = CVAT_DIR / zip_name

        print(f"[ZIP {package_index:03d}] Creating {zip_name} | images={len(chunk)}")

        # JPG đã nén rồi, dùng ZIP_STORED cho nhanh
        with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_STORED) as zf:
            for img in chunk:
                # Tên ảnh đã có prefix clip: hiv00000_000001.jpg
    ...
    with SUMMARY_TXT.open("w", encoding="utf-8") as f:
        f.write("==================================================\n")
        f.write("SAFE DRIVE PHONE/RADIO - CVAT ZIP PACKAGES\n")
        f.write("==================================================\n")
